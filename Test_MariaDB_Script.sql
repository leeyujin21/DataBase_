...
SELECT P.PDNAME AS "제품카테고리",
       ...
       F.FACNAME AS "공장명",
       ST.STONAME AS "판매점명",
       COALESCE(ST.STAMOUNT, 0) AS "판매점 재고 수량"
FROM PRODUCT P
JOIN FACTORY F ON P.FACTNO = F.FACTNO
JOIN STORE ST ON P.PDNO = ST.PDNO
WHERE F.FACLOC = 'SEOUL'
      AND (ST.STAMOUNT = 0 OR ST.STAMOUNT IS NULL)
ORDER BY 1;

-- 2번 문제 
SELECT P.PDSUBNAME, P.PDNAME, P.PDCOST, P.PDPRICE
FROM PRODUCT P
WHERE P.PDCOST > (SELECT MIN(PDCOST) FROM product WHERE PDNAME='TV')
AND P.PDCOST < (SELECT MAX(PDCOST) FROM product WHERE PDNAME='CELLPHONE')
ORDER BY 3;
...
SELECT PDNO, PDNAME, PDSUBNAME, FACTNO, PDDATE, PDCOST, PDPRICE, PDAMOUNT, CURRENT_DATE
FROM PRODUCT P
WHERE P.FACTNO IN (SELECT FACTNO FROM FACTORY WHERE FACLOC = 'CHANGWON');
...
DELETE
FROM product 
WHERE FACTNO IN (SELECT FACTNO FROM FACTORY WHERE FACLOC = 'CHANGWON');
